...
#####################################################################
  ...
  {
    value_range <- range(0, chiao_talk$ID,ilan_talk$ID,lotung_talk$ID,suao_talk$ID,toucheng_talk$ID)
    
    plot(chiao_talk$ID, type="o", col="blue", ylim=value_range,axes=FALSE, ann=FALSE)
    
    axis(1, at=1:24, lab=ilan$time)
    axis(2, las=1, at=50*0:value_range[2])
    
    box()
    
    lines(ilan_talk$ID, type="o", lty=1, col="red")
    lines(lotung_talk$ID, type="o", lty=1, col="#00DB00")
    lines(suao_talk$ID, type="o", lty=1, col="#272727")
    lines(toucheng$ID, type="o", lty=1, col="purple")
